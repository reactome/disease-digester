...
let overlay = new Vue({
    ...
    created: function () {
            axios.get('/overlays/open-target/findAll?pageNumber=1&pageSize=50&geneSize=10&score=0.0&sort=GENE&order=DESC')
            .then(res => this.setData(res.data))
            .catch(err => console.log(err));
        axios.get('/overlays/open-target/diseaseNameHintWord')
            .then(res => {
    ...
    methods: {
        ...
        loadData: function (func = '/findAll', pageNumber, pageSize, geneSize, score, sort = this.sort, order = this.order) {
            ...
            if (func.match('/findAll')) {
                url = '/overlays/open-target' + func + '?pageNumber=' + pageNumber + '&pageSize=' + pageSize + '&geneSize=' + geneSize + '&score=' + score + '&sort=' + sort + '&order=' + order;
            } else {
                url = '/overlays/open-target' + func + '&pageNumber=' + pageNumber + '&pageSize=' + pageSize + '&geneSize=' + geneSize + '&score=' + score + '&sort=' + sort + '&order=' + order;
            }
            console.log(url);
        ...
        getMaxGeneSize() {
            let maxGeneSizeUrl = '/overlays/open-target/getMaxGeneSize?score=' + this.score;
            if (this.name != null) {
        ...
        async analyze(geneItems) {
            ...
            analyzeWindow.focus();
            await axios.post('/overlays/open-target/analyze', data)
                .then(res => {
                    url = res.data;
                }).catch(err => {
                    console.log(err)
