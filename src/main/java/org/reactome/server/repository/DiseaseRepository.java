...
public interface DiseaseRepository extends CrudRepository<Disease, String> {
    ...
    @Query(value =
            "        SELECT g.accessionNumber as acc, " +
                    ...
                    "WHERE d.source = 0 AND g.accessionNumber IN :geneAcs " +
    ...
    @Query(value =
            "        SELECT g.accessionNumber as acc, " +
                    ...
                    "               'alias', d.diseaseName, " +
                    "               'score', TRUNCATE(gda.score, 3), " +
                    "               'accURL', CONCAT('https://platform.opentargets.org/disease/', d.diseaseId), " +
                    "               'evidencesURL', CONCAT('https://platform.opentargets.org/evidence/', g.geneId, '/', d.diseaseId) " +
                    "               ) " +
                    "           ) as interactors " +
                    "FROM disease d " +
                    "         INNER JOIN gda gda on d.diseaseId = gda.diseaseId " +
                    "         INNER JOIN gene g on gda.geneId = g.geneId " +
                    "WHERE d.source = 1 AND g.accessionNumber IN :geneAcs " +
                    "GROUP BY g.accessionNumber ", nativeQuery = true)
    List<Tuple> findByGenesOpenTarget(@Param("geneAcs") Set<String> geneAcs);
    ...
    @Query("select count(d.diseaseId), count(gda.gene) as associatedGenesCount " +
            "from Disease d " +
            "inner join GDA gda on d.diseaseId = gda.disease " +
            "inner join Gene g on gda.gene = g.geneId " +
            "where gda.score >= :score " +
