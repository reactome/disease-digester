...
public class OpenTargetParser implements Parser {
    ...
    public OpenTargetParser() {
        try (Stream<String> lines = Files.lines(Path.of(ClassLoader.getSystemResource("genetics_target_list.json").toURI()), Charset.defaultCharset())) {
            lines.forEach(s -> {
                try {
