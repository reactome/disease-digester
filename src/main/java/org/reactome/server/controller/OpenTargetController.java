...
import org.reactome.server.service.OpenTargetHintWordService;
...
public class OpenTargetController {
    ...
    private final OpenTargetHintWordService hintWordService;

    @Autowired
    public OpenTargetController(DiseaseService diseaseService, OpenTargetHintWordService hintWordService) {
    ...
    DiseaseResult findAll(
        ...
        return diseaseService.findAll(pageNumber, pageSize, score, geneSize, SourceDatabase.OPEN_TARGET, sortBy, order);
    }
    ...
    DiseaseResult findByDiseaseName(@RequestParam("name") String diseaseName,
        ...
        return diseaseService.findByDiseaseName(pageNumber, pageSize, diseaseName, score, geneSize, SourceDatabase.OPEN_TARGET, sort, order);
    }
    ...
    Long getMaxGeneSize(@RequestParam("score") Float score, @RequestParam(value = "name", required = false) String name) {
        return Optional.ofNullable(diseaseService.getMaxGeneSize(score, name, SourceDatabase.OPEN_TARGET)).orElse(0L);
    }
